...
int main() {
    ...
    for (int x = 0; x < 3; x++) {
        set<char> set;
        for (int y = 0; y < 3; y++) set.insert(board[x + 3 * y]);
    ...
    for (int y = 0; y < 3; y++) {
        set<char> set;
        for (int x = 0; x < 3; x++) set.insert(board[x + 3 * y]);
        if (set.size() == 2) add_team(pair(*set.begin(), *++set.begin()));
        else if (set.size() == 1) add_idvl(*set.begin());
    }
    ...
    set<char> dec_diag;
    for (int n = 0; n < 3; n++) dec_diag.insert(board[4 * n]);
    if (dec_diag.size() == 2)
    ...
    set<char> inc_diag;
    for (int n = 0; n < 3; n++) inc_diag.insert(board[2 * n + 2]);
    if (inc_diag.size() == 2)
